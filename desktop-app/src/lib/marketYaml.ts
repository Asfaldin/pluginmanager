...
export interface MarketConfig {
  ...
  /** Rozmiar okna (9-54). */
  size: number;
  /** Pola na oferty w kolejności wypełniania (menu.offer-slots). */
  offerSlots: number[];
  /** Więcej ofert dla rang (limits.ranks): uprawnienie mainplugins.market.rank.<rank>. */
  rankLimits: RankLimit[];
  raw: Obj;
}
...
/** Domyślne miejsca na oferty - blok 7x3 w środku okna (jak w pluginie). */
export const DEFAULT_OFFER_SLOTS = [10, 11, 12, 13, 14, 15, 16, 19, 20, 21, 22, 23, 24, 25, 28, 29, 30, 31, 32, 33, 34];
...
export function defaultMarket(): MarketConfig {
  return {
    ...
    size: 54,
    offerSlots: [...DEFAULT_OFFER_SLOTS],
    rankLimits: [],
    raw: {},
  };
}
...
export function parseMarketYaml(text: string): MarketConfig {
  ...
  return {
    ...
    buttons,
    size: [9, 18, 27, 36, 45, 54].includes(num(menu.size, 0)) ? num(menu.size, 54) : d.size,
    offerSlots: Array.isArray(menu["offer-slots"])
      ? [...new Set((menu["offer-slots"] as unknown[]).filter((x): x is number => typeof x === "number"))]
      : d.offerSlots,
    rankLimits: Object.entries(obj(obj(raw.limits).ranks)).map(([rank, v]) => ({ rank, limit: num(v, 1) })),
    raw,
  };
}
...
export function serializeMarketYaml(c: MarketConfig): string {
  ...
  const out: Obj = {
    ...
    limits: {
      ...obj(c.raw.limits),
      default: c.defaultLimit,
      ranks: Object.fromEntries(c.rankLimits.filter((r) => r.rank.trim()).map((r) => [r.rank.trim().toLowerCase(), r.limit])),
    },
    ...
    menu: { ...menu, title: c.title, background: c.background, size: c.size, "offer-slots": [...c.offerSlots].sort((a, b) => a - b), buttons },
  };
...
export function marketProblems(c: MarketConfig): string[] {
  ...
  for (const id of BUTTON_IDS) {
    ...
    if (b.slot < 0 || b.slot >= c.size) out.push(`Przycisk „${name}” stoi poza oknem - nie pokaże się.`);
    else if (c.offerSlots.includes(b.slot)) out.push(`Przycisk „${name}” stoi na polu ofert (${b.slot}) - nie pokaże się.`);
    else if (used.has(b.slot)) out.push(`Przyciski „${used.get(b.slot)}” i „${name}” są na tym samym polu (${b.slot}).`);
    else used.set(b.slot, name);
  }
  if (c.offerSlots.filter((s) => s < c.size).length === 0) out.push("W oknie nie ma żadnego pola na oferty - gracze nic nie zobaczą.");
